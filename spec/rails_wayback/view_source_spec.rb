...
RSpec.describe RailsWayback::ViewSource do
  ...
  it "automatically prunes the least-recently-used ref after a historical render" do
    SpecSupport.with_tmp_root do |root|
      SpecSupport.build_git_repo(root)
      SpecSupport.commit_file(root, "app/views/home/index.html.erb", "<h1>v1</h1>", message: "v1")
      SpecSupport.commit_file(root, "app/views/home/index.html.erb", "<h1>v2</h1>", message: "v2")
      ...
      oldest_sha = commits.last.sha
      source = described_class.new(configuration: RailsWayback.configuration, git: git)
      oldest = source.materialize(oldest_sha)
      marker = oldest.join(RailsWayback::CacheInventory::MARKER_NAME)
      old_access = Time.at(1).utc
      File.utime(old_access, old_access, marker)
      RailsWayback.configuration.max_cached_refs = 1
      RailsWayback.configuration.max_cache_bytes = nil

      result = source.with_view_roots(newest_sha) do |roots|
        expect(roots.first.join("home/index.html.erb").read).to include("v2")
        expect(oldest).to exist
        :rendered
      end

      expect(result).to eq(:rendered)
      expect(oldest).not_to exist
      expect(RailsWayback.configuration.refs_cache_path.join(newest_sha)).to exist
    end
  end

  it "keeps a cached ref leased until rendering finishes before pruning it" do
    SpecSupport.with_tmp_root do |root|
      SpecSupport.build_git_repo(root)
      SpecSupport.commit_file(root, "app/views/home/index.html.erb", "<h1>v1</h1>", message: "v1")
      ...
      RailsWayback.configuration.max_cache_bytes = nil
      lease_started = Queue.new
      release_lease = Queue.new
      prune_result = Queue.new

      rendering = Thread.new do
        source.with_view_roots(sha) do
          lease_started << true
          release_lease.pop
        end
      end
